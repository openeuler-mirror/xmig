...
fn generate_all_bindings(include_paths: &[&Path]) {
    ...
    let cuda_include_path =
        Path::new(&env::var("CUDA_HOME").unwrap_or(CUDA_HOME.to_string())).join("include");
    let nccl_src_include_path =
        Path::new(&env::var("NCCL_SRC_HOME").unwrap_or(NCCL_SRC_HOME.to_string()))
            .join("src")
            .join("include");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let cuda_home = env::var("CUDA_HOME").unwrap_or(CUDA_HOME.to_string());
    let cuda_home = Path::new(&cuda_home);
    let nccl_src_home = env::var("NCCL_SRC_HOME").unwrap_or(NCCL_SRC_HOME.to_string());
